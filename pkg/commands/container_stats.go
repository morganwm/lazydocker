...
// CalculateContainerMemoryUsage calculates the memory usage in MB
func (s *ContainerStats) CalculateContainerMemoryUsageMB() float64 {

	value := float64(s.MemoryStats.Usage) / float64(1048576)
	if math.IsNaN(value) {
		return 0
	}
	return value
...
func (c *Container) PlotGraph(spec config.GraphConfig, width int) (string, error) {
	...
	if spec.StatPath == "DerivedStats.MemoryUsageMB" {
		prettyMemVal := utils.ByteCountBinary(int64(data[len(data)-1] * float64(1048576)))
		...
		return asciigraph.Plot(
			...
			asciigraph.Min(min),
			asciigraph.Max(max),
			asciigraph.Caption(fmt.Sprintf("%s: %s (%v)", spec.Caption, prettyMemVal, time.Since(c.StatHistory[0].RecordedAt).Round(time.Second))),
		), nil
	}
